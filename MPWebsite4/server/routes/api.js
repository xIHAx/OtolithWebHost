...
{  
    ...
    {    
        db.collection('users').insertOne({"name" : username, "password" : hash, "role" : role, "email" : email, "mobile": mobile, "address": address, "greenCurrency": 0, "collected": false, "verified": false, "temporaryToken":temporaryToken, "passwordToken":false},
        (err, result) => {   
        if (err) return console.log(err)   
