...
{  
    ...
    let mailOptions = {
        to: ['otolithmp@gmail.com'], // Enter here the email address on which you want to send emails from your customers
        from: 'otolithmp@gmail.com',
        subject: 'Otolith Account Authenticate',
        html:
        '<h2>Please Verify This Email Address</h2>' + 
        'Hi ' + username +',<br><p>Please enter this token: ' + temporaryToken +
    ...
    {    
        db.collection('users').insertOne({"name" : username, "password" : hash, "role" : role, "email" : email, "mobile": mobile, "address": address, "greenCurrency": 0, "collected": false, "verified": false, 'temporaryToken':temporaryToken},
        (err, result) => {   
        if (err) return console.log(err)   
